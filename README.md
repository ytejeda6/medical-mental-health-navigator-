...
AI Capability:
I used the text generation as the primary capability. The failure point is a language and access barrier, as the patient cannot get a
...
same language the user writes in") determines who the tool can serve — which is itself a policy decision, not just a technical setting, as Lab 1 demonstrated. I used structured extraction as a supporting capability to keep provider data 
...
AI Processing: A text generation model acts as a bilingual mental health navigator. The system prompt instructs it to always respond in the user's language, use plain non-clinical language, and only recommend providers from a pre-loaded list.

AI Processing: Structured extraction reads raw provider directory text and outputs a consistent five-field JSON record (provider_name, languages_spoken, specialties, accepts_medi_cal, phone) so the navigator always has accurate data.
...
Component 1: Conversational Bilingual Routing Engine(Spanish response)
<img width="2696" height="1220" alt="image" src="https://github.com/user-attachments/assets/1ea3ea36-5a4e-4553-b8d8-4c9c60786b02" />

Screenshot — Component 2: Automated Provider Directory Ingestion Pipeline
